 ...
 * Clase Batalla.
 * <br>
 * Descripcion: Clase que implementa la interfaz IState para el uso del patron State. Se encarga de decidir los resultados de la batalla y asignar las respectivas recompensas.<br>
 */
public class Definicion implements IState
{
	...
	public void correr()
	{
		...
		mensaje += this.arena.getPerdedor().getNombre() + " queda eliminado del torneo con " + this.arena.getPerdedor().getNombre() + " rondas ganadas.\n";
		...
		if (this.arena.getCartaGanada() != null)
		{
			...
			mensaje += this.arena.getGanador().getNombre() + " recibe la carta " + this.arena.getCartaGanada() + " usada por " + this.arena.getPerdedor().getNombre() + ".\n";
		}
